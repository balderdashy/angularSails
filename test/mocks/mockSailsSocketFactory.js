angular.module('angularSails.mocks',[]).factory('$sailsSocketFactory',['$rootScope',function($rootScope){
    ...
    function MockIOEmitExpectation(eventName,data){
        this.eventName = eventName.toLowerCase();
        this.data = data;
    }


    function MockIOSocket(url){
        this._url = url;
    }

    MockIOSocket.prototype.emit = function(eventName,data,callback){
        console.log('emitting')
        emitQueue.push({eventName: eventName, data: data, callback: callback || false})
    }

    MockIOSocket.prototype.on = function(eventName,handler){
    ...
    MockIOSocket.prototype.addListener = function(eventName,handler){
        if(!listeners[eventName]){
            listeners[eventName] = [];
        }
        listeners[eventName].push(handler);
    ...
    MockIOSocket.prototype.once = function(eventName,handler){
        if(!listeners[eventName]){
            listeners[eventName] = [];
        }
        listeners[eventName].push(handler);
    ...
    MockIOSocket.prototype.receive = function(eventName){
    ...
    mockSocketFactory.flush = function(){
        $rootScope.$digest();
        while (request = emitQueue.shift()) {

            angular.forEach(pendingEmits,function(expectedEmit,i){
                if(request.eventName == expectedEmit.eventName && request.data.url == expectedEmit.data.url && !expectedEmit.resolved){
                    expectedEmit.resolved = true;
                    pendingEmits.splice(i,1);
                    if(request.callback){
                        request.callback(expectedEmit.response);
                    }
                }
            })
    ...
    mockSocketFactory.expectEmit = function (eventName,data) {

        if(typeof data == 'string'){
            data = {url: data};
        }
        var mockEmit = new MockIOEmitExpectation(eventName,data);
        pendingEmits.push(mockEmit);

        return {
            respond: function(statusCode,data){
                mockEmit.response = {statusCode: statusCode, body: data}
            }
        }
    };

    mockSocketFactory.verifyNoOutstandingExpectation = function() {
      $rootScope.$digest();
      if (pendingEmits.length) {
          console.log(pendingEmits)
        throw new Error('Unsatisfied requests: ' + pendingEmits.join(', '));
      }
    };
     ...
     * ```
     */
    mockSocketFactory.verifyNoOutstandingRequest = function() {
      if (emitQueue.length) {
        throw new Error('Unflushed requests: ' + emitQueue.length);
      }
    };
